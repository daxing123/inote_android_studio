...
public class ImapStore extends Store {
    ...
    public static final int NOT_NEED_CREATE_STARTED_TIME = -1;
    public static final String XOAUTH2_400 = "XOAUTH2 400";
    ...
    /**
     * Cache of ImapFolder objects. ImapFolders are attached to a given folder on the server and as long as their
     * associated connection remains open they are reusable between requests. This cache lets us make sure we always
     * reuse, if possible, for a given folder name.
     */
    private HashMap<String, ImapFolder> mFolderCache = new HashMap<String, ImapFolder>();

    /**
     * imap://auth:user:password@server:port CONNECTION_SECURITY_NONE imap+tls://auth:user:password@server:port
     * CONNECTION_SECURITY_TLS_OPTIONAL imap+tls+://auth:user:password@server:port CONNECTION_SECURITY_TLS_REQUIRED
    ...
    public List<? extends Folder> getPersonalNamespaces(boolean forceListAll) throws MessagingException {
        ...
        try {
            ...
            connection.close();
            throw new MessagingException("Unable to get folder list.\n" + ioe.getMessage(), ioe);
        } catch (AuthenticationFailedException ae) {
            connection.close();
            throw new AuthenticationFailedException("Failed to login " + ae.getMessage());
    ...
    private List<? extends Folder> listFolders(ImapConnection connection, boolean LSUB) throws IOException,
        ...
        List<ImapResponse> responses = connection.executeSimpleCommand(String.format(commandResponse + " \"\" %s",
        ...
        for (ImapResponse response : responses) {
            if (ImapResponseParser.equalsIgnoreCase(response.get(0), commandResponse)) {
                ...
                if (folder.equalsIgnoreCase(mAccount.getInboxFolderName())) {
                    ...
                    /*
                     * There is a folder on the server with the same name as our local outbox. Until we have a good plan
                     * to deal with this situation we simply ignore the folder on the server.
                     */
                    continue;
                } else {
    ...
    @Override
    public Folder getGmailTrashFolder() throws IOException, MessagingException {
        ImapConnection connection = getConnection();

        String commandResponse = "XLIST";

        List<ImapResponse> responses = connection.executeSimpleCommand(String.format(commandResponse + " \"\" %s",
                encodeString(getCombinedPrefix() + "*")));

        for (ImapResponse response : responses) {
            Log.d(K9.LOG_TAG, " ##### getGmailTrashFolder response :" + response.toString());
            if (ImapResponseParser.equalsIgnoreCase(response.get(0), commandResponse)) {
                String decodedFolderName;
                try {
                    decodedFolderName = decodeFolderName(response.getString(3));
                ...
                for (int i = 0, count = attributes.size(); i < count; i++) {
                    ...
                    if (attribute.equalsIgnoreCase("\\Trash")) {
                        Log.d(K9.LOG_TAG, "********* getGmailTrashFolder equalsIgnoreCase trash success ********");
                        return getFolder(decodedFolderName);
                    }
                }
            }
        }
    ...
    public void checkSettings() throws MessagingException {
        try {
            ...
            throw new MessagingException(ioe.getLocalizedMessage(), ioe);
        }
    }
...
/**
    ...
    private String encodeFolderName(String name) {
        try {
            ...
            /*
             * The only thing that can throw this is getBytes("US-ASCII") and if US-ASCII doesn't exist we're totally
             * screwed.
             */
    ...
    private String decodeFolderName(String name) throws CharacterCodingException {
        /*
         * Convert the encoded name to US-ASCII, then pass it through the modified UTF-7 decoder and return the Unicode
         * String.
         */
        try {
    ...
    class ImapFolder extends Folder {
        ...
        public List<ImapResponse> internalOpen(OpenMode mode) throws MessagingException {
            ...
            try {
                ...
                /*
                 * If the command succeeds we expect the folder has been opened read-write unless we are notified
                 * otherwise in the responses.
                 */
                mMode = mode;
         ...
         * 
         * @param folderName
         *            The name of the folder encoded as quoted string. See {@link ImapStore#encodeString}
         * 
        ...
        public boolean exists() throws MessagingException {
            ...
            /*
             * This method needs to operate in the unselected mode as well as the selected mode so we must get the
             * connection ourselves if it's not there. We are specifically not calling checkOpen() since we don't care
        ...
        public void copyMessages(Message[] messages, Folder folder) throws MessagingException {
            ...
            try {
                ...
                if (!exists(remoteDestName)) {
                     ...
                     */
                    if (K9.DEBUG)
                        Log.i(K9.LOG_TAG, "IMAPMessage.copyMessages: attempting to create remote '" + remoteDestName
                                + "' folder for " + getLogId());
                    iFolder.create(FolderType.HOLDS_MESSAGES);
                }

                if (exists(remoteDestName)) {
                    executeSimpleCommand(String.format("UID COPY %s %s", Utility.combine(uids, ','), remoteDestName));
                } else {
                    throw new MessagingException("IMAPMessage.copyMessages: remote destination folder "
                            + folder.getName() + " does not exist and could not be created for " + getLogId(), true);
                }
            } catch (IOException ioe) {
                throw ioExceptionHandler(mConnection, ioe);
            }
        ...
        public void delete(Message[] messages, String trashFolderName) throws MessagingException {
            ...
            if (trashFolderName == null || getName().equalsIgnoreCase(trashFolderName)) {
                ...
                if (!exists(remoteTrashName)) {
                    /*
                     * If the remote trash folder doesn't exist, just remove the label.
                     */
                    // if(remoteTrashName.startsWith("Trash")) {
                    // if (K9.DEBUG)
                    // Log.i(K9.LOG_TAG,
        ...
        @Override
        public void setLabel(Message[] messages, String label, boolean value) throws MessagingException {
            checkOpen();
            String[] uids = new String[messages.length];
            for (int i = 0, count = messages.length; i < count; i++) {
                uids[i] = messages[i].getUid();
            ...
            try {
                executeSimpleCommand(String.format("UID STORE %s %sX-GM-LABELS (\"%s\")", Utility.combine(uids, ','),
        ...
        public void rename(String oldName, String newName) throws MessagingException {
            checkOpen();
            try {
                executeSimpleCommand(String.format("RENAME %s %s", oldName, newName));
            } catch (IOException ioe) {
            ...
            ImapSearcher searcher = new ImapSearcher() {
                public List<ImapResponse> search() throws IOException, MessagingException {
                    return executeSimpleCommand(String.format("UID SEARCH UID %s"
                            + (includeDeleted ? "" : " NOT DELETED"), Utility.combine(mesgUids.toArray(), ',')));
                }
            };
            return search(searcher, listener).toArray(EMPTY_MESSAGE_ARRAY);
        }

        private List<Message> search(ImapSearcher searcher, MessageRetrievalListener listener)
                throws MessagingException {
        ...
        public void fetch(Message[] messages, FetchProfile fp, MessageRetrievalListener listener)
            ...
            /*
             * Figure out what command we are going to run: Flags - UID FETCH (FLAGS) Envelope - UID FETCH ([FLAGS]
             * INTERNALDATE UID RFC822.SIZE FLAGS BODY.PEEK[HEADER.FIELDS (date subject from content-type to cc)])
             */
        ...
        @Override
        public int fetchMessageSize(Message message) throws MessagingException {
            if (message != null) {
                checkOpen();
                 ...
                 * INTERNALDATE UID RFC822.SIZE FLAGS BODY.PEEK[HEADER.FIELDS (date subject from content-type to cc)])
                 */
                LinkedHashSet<String> fetchEnvelopeFields = new LinkedHashSet<String>();
                fetchEnvelopeFields.add("UID");
                fetchEnvelopeFields.add("INTERNALDATE");
                fetchEnvelopeFields.add("RFC822.SIZE");
                fetchEnvelopeFields.add("BODY.PEEK[HEADER.FIELDS (date subject from content-type to cc reply-to "
                        + K9.IDENTITY_HEADER + ")]");
                ...
                try {
                    ...
                    mConnection.sendCommand(String.format("UID FETCH %s (%s)",
                            Utility.combine(new String[] { uid }, ','),
                            Utility.combine(fetchEnvelopeFields.toArray(new String[fetchEnvelopeFields.size()]), ' ')),
                            false);
                    do {
                        callback = new FetchBodyCallback(messageMap);
                        response = mConnection.readResponse(callback);
                        ...
                        if (response.mTag == null && ImapResponseParser.equalsIgnoreCase(response.get(1), "FETCH")) {
                            ...
                            return fetchList.getKeyedNumber("RFC822.SIZE");
                        } else {
                            handleUntaggedResponse(response);
                        }
                    } while (response.mTag == null);
        ...
        @Override
        public void fetchFilterLarge(Message[] messages, FetchProfile fp, long maxSize,
                ...
                try {
                    ...
                    do {
                        ...
                        if (response.mTag == null && ImapResponseParser.equalsIgnoreCase(response.get(1), "FETCH")) {
                            ...
                            long rfc822_size = fetchList.getKeyedNumber("RFC822.SIZE");
                            if (rfc822_size > maxSize) {
                                fetchBody = false;

                                String uid = fetchList.getKeyedString("UID");
                                Message message = messageMap.get(uid);
                                listener.messagefilteredByLarge(message == null ? "" : message.getSubject().trim());
                            }
                        } else {
                            handleUntaggedResponse(response);
                        }
        ...
        private void parseBodyStructure(ImapList bs, Part part, String id) throws MessagingException {
            if (bs.get(0) instanceof ImapList) {
                ...
                /*
                 * 0| 0 body type 1| 1 body subtype 2| 2 body parameter parenthesized list 3| 3 body id (unused) 4| 4
                 * body description (unused) 5| 5 body encoding 6| 6 body size -| 7 text lines (only for type TEXT,
                 * unused) Extensions (optional): 7| 8 body MD5 (unused) 8| 9 body disposition 9|10 body language
                 * (unused) 10|11 body location (unused)
                 */
                ...
                if (bodyDisposition != null && bodyDisposition.size() > 0) {
                    ...
                    if ((bodyDisposition.size() > 1) && (bodyDisposition.get(1) instanceof ImapList)) {
                        ...
                        /*
                         * If there is body disposition information we can pull some more information about the
                         * attachment out.
                         */
                        for (int i = 0, count = bodyDispositionParams.size(); i < count; i += 2) {
                            contentDisposition += String.format(";\n %s=\"%s\"", bodyDispositionParams.getString(i)
                                    .toLowerCase(), bodyDispositionParams.getString(i + 1));
                        }
                    }
                }
                ...
                /*
                 * Set the content disposition containing at least the size. Attachment handling code will use this down
                 * the road.
                 */
        ...
        /**
         * Appends the given messages to the selected folder. This implementation also determines the new UID of the
         * given message on the IMAP server and sets the Message's UID to the new server UID.
         */
        @Override
        public void appendMessages(Message[] messages, AppendMessagesListener listener) throws MessagingException {
            appendMessages(NOT_NEED_CREATE_STARTED_TIME, messages, listener);
        }

        @Override
        public void appendMessages(long startedTime, Message[] messages, AppendMessagesListener listener)
                throws MessagingException {
            checkOpen();
            try {
                Message message = null;
                for (int i = 0, total = messages.length; i < total; i++) {
                    message = messages[i];
                    ...
                    if (response.size() > 1) {
                        /*
                         * If the server supports UIDPLUS, then along with the APPEND response it will return an
                         * APPENDUID response code, e.g.
                         * 
                        ...
                        if (responseList instanceof ImapList) {
                            ...
                            if (appendList.size() >= 3 && appendList.getString(0).equals("APPENDUID")) {
                                ...
                                if (newUid != null && newUid.trim().length() > 0) {
                                    message.setUid(newUid);
                                    if (listener != null) {
                                        String messageId = null;
                                        String[] messageIdHeader = message
                                                .getHeader(MimeHeader.HEADER_X_UNIVERSALLY_UNIQUE_IDENTIFIER);
                                        if (messageIdHeader != null && messageIdHeader.length > 0) {
                                            messageId = messageIdHeader[0];
                                        }
                                        listener.messageFinished(newUid, messageId, i, total, startedTime);
                                    }
                                    continue;
                                }
                            }
        ...
        public String getUidFromMessageId(Message message) throws MessagingException {
            try {
                /*
                 * Try to find the UID of the message we just appended using the Message-ID header.
                 */
                ...
                // get from gmail trash
                responses = executeSimpleCommand(String.format("UID SEARCH X-GM-RAW \"rfc822msgid:%s\"", messageId));
                for (ImapResponse response1 : responses) {
                    if (response1.mTag == null && ImapResponseParser.equalsIgnoreCase(response1.get(0), "SEARCH")
                            && response1.size() > 1) {
                        return response1.getString(1);
                ...
                Log.e(K9.LOG_TAG, "ImapException when get uid from messageid");
                return null;
            }
        }

        @Override
        public String[] getLabelsByUid(String uid) throws MessagingException {
            try {
                /*
                 * Try to find the UID of the message we just appended using the Message-ID header.
                 */
                ...
                if (K9.DEBUG)
                    Log.d(K9.LOG_TAG, "Looking for LABELS for message with uid " + uid + " for " + getLogId());
                List<ImapResponse> responses = executeSimpleCommand(String.format("UID FETCH %s (X-GM-LABELS)", uid));
                for (ImapResponse response1 : responses) {
                    if (response1.mTag == null && ImapResponseParser.equalsIgnoreCase(response1.get(1), "FETCH")
                            && response1.size() >= 3) {
                        ImapList list = response1.getList(2);
                        if (list.size() >= 2) {
                            ImapList labels = list.getList(1);
                ...
                throw new MessagingException("Could not find labels for message based on Message-ID", ioe);
            } catch (ImapException e) {
        ...
        public void setFlags(Message[] messages, Flag[] flags, boolean value) throws MessagingException {
            ...
            try {
                executeSimpleCommand(String.format("UID STORE %s %sFLAGS.SILENT (%s)", Utility.combine(uids, ','),
                        value ? "+" : "-", Utility.combine(flagNames.toArray(new String[flagNames.size()]), ' ')));
            } catch (IOException ioe) {
                throw ioExceptionHandler(mConnection, ioe);
            }
        }
    ...
    public static class ImapConnection {
        ...
        public void open() throws IOException, MessagingException {
            ...
            try {
                ...
                if (K9.DEBUG)
                    Log.i(K9.LOG_TAG, "Connection " + getLogId() + " connecting to " + mSettings.getHost()
                ...
                if (mSettings.getConnectionSecurity() == CONNECTION_SECURITY_SSL_REQUIRED
                    ...
                    sslContext.init(null, new TrustManager[] { TrustManagerFactory.get(mSettings.getHost(), secure) },
                            new SecureRandom());
                ...
                try {
                    ...
                    } else if (mSettings.getAuthType() == AuthType.XOAUTH) {
                        receiveCapabilities(executeSimpleCommand(
                                String.format("AUTHENTICATE XOAUTH2 " + new String(mSettings.getPassword())), false));
                    }
                    authSuccess = true;
                } catch (ImapException ie) {
                    throw new AuthenticationFailedException(ie.getAlertText(), ie);
                } catch (MessagingException me) {
                    throw new AuthenticationFailedException(me.getMessage(), me);
                }
                if (K9.DEBUG) {
        ...
        protected void authCramMD5() throws AuthenticationFailedException, MessagingException {
            try {
                ...
                // String authToken =
                // String tag =
                // sendCommand("AUTHENTICATE XOAUTH DQAAALAAAAApePvTeboTkmAq5uv7jUSSgxYA8ugwQAvB4VQqts-bQCCvoY7elvWEF7NUtCjzor-euDZW0qqEfkm38QnC0H56oSdR8jCOxGZlzF_OT9pkbI0A4Y01q7DXXLgKc44MUS5feV5aQ_X1FsGelVVU9hInVqgD-3axQYXk4-9iyVzfjGSsgNxcgHJBBm-hRYqIeUjWck_qgLRI75PU9gLtm_uL1o68zd4B462ZIR7NJrD1kQ",
                // false);
        ...
        public List<ImapResponse> executeSimpleCommand(String command, boolean sensitive,
            ...
            do {
                ...
                if (response.mTag != null && !response.mTag.equalsIgnoreCase(tag)) {
                    ...
                    response.mTag = null;
                    continue;
                } else if (command.contains("XOAUTH2")) {
                    String status = response.getXOauth2Status();
                    if ("400".equals(status)) {
                        throw new AuthenticationFailedException(XOAUTH2_400);
                    }
                    Log.d(K9.LOG_TAG, "XOAUTH2 response status:" + String.valueOf(status));
                }
