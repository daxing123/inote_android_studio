...
public class ImapResponseParser {
    ...
    public ImapResponse readResponse(IImapResponseCallback callback) throws IOException {
        try {
            ...
            if (mException != null) {
                throw new RuntimeException("readResponse(): Exception in callback method",
                        mException);
            }
    ...
    public class ImapResponse extends ImapList {
        ...
        public String getXOauth2Status() {
            if (size() > 0) {
                String oauth2response = edcodeBase64(getString(0).getBytes());
                try {
                    JSONObject json = new JSONObject(oauth2response);
        ...
        private String edcodeBase64(byte[] params) {
            try {
                byte[] arrayOfByte = Base64.decodeBase64(params);
    ...
    public boolean isStatusResponse(String symbol) {
        return symbol.equalsIgnoreCase("OK") || symbol.equalsIgnoreCase("NO")
                || symbol.equalsIgnoreCase("BAD") || symbol.equalsIgnoreCase("PREAUTH")
